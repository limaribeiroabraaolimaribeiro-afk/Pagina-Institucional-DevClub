import { Play, Search, ShoppingCart, Send, ArrowUp, ArrowDown } from 'lucide-react'
    ...
    <div className="relative flex h-full w-full flex-col overflow-hidden bg-[#0a0e14] text-[7px] leading-none">
        ...
        <div className="flex flex-1 flex-col gap-2 p-2.5">
          <div className="flex items-center justify-between rounded-md bg-white/[0.04] px-2 py-1.5">
            <div className="flex items-center gap-1">
              <span className={`size-2 rounded-sm ${strong}`} />
          ...
          <div className="grid flex-1 grid-cols-3 gap-1.5">
            {[0, 1, 2].map((tile) => (
              <div key={tile} className="flex flex-col gap-1 rounded-md bg-white/[0.04] p-1.5">
                <div className={`flex-1 rounded ${tile === 1 ? soft : 'bg-white/[0.07]'}`} />
      ...
      {type === 'streaming' && (
        <div className="flex flex-1 flex-col gap-1.5 p-2.5">
          ...
          <div className={`relative flex h-9 flex-col justify-end overflow-hidden rounded-md bg-linear-to-br p-1.5 ${accent === 'green' ? 'from-green/30 via-void to-purple/20' : 'from-purple/30 via-void to-green/20'}`}>
            <div className="h-1 w-16 rounded-full bg-white/40" />
            <span className={`mt-1 inline-flex w-fit items-center gap-1 rounded px-1.5 py-0.5 font-semibold text-void ${strong.replace('/40', '')}`}>
      ...
      {type === 'course' && (
        <div className="flex flex-1 gap-1.5 p-2.5">
          <div className="flex w-6 shrink-0 flex-col gap-1">
            {[0, 1, 2, 3].map((dot) => (
              <span key={dot} className={`h-1 rounded-full ${dot === 1 ? strong : 'bg-white/10'}`} />
            ))}
          </div>
          <div className="flex flex-1 flex-col gap-1.5">
            <div className={`flex flex-1 items-center justify-center rounded-md ${soft}`}>
              <Play className={`size-3.5 ${text}`} fill="currentColor" aria-hidden="true" />
            </div>
            <div className="h-1 w-full overflow-hidden rounded-full bg-white/10">
              <div className={`h-full w-2/3 rounded-full ${strong}`} />
            </div>
            <div className="flex items-center gap-1 text-white/40">
              <span className={text}>✓</span> Aula 4 de 12
            </div>
          </div>
        </div>
      )}
